    ...
    create()
    {
        document.querySelectorAll(this.id).forEach(element => 
        {
            ...
            element.addEventListener("mousemove", e => 
            {
                ...
                element.style.setProperty("transform", perspective + propXRot + propYRot + "scale(1.2)");
            })

            element.addEventListener("mouseleave", e => 
            {
                element.style.setProperty("transition", "transform " + this.release + "s");
                element.style.setProperty("transform", perspective + "rotateX(0.15deg) rotateY(-0.15deg)");
            })
        })
    }
